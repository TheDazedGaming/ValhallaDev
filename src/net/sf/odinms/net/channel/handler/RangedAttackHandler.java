...
//		throw new RuntimeException("[h4x] Player " + c.getPlayer().getName() + " is attacking with something that's not a weapon");
		return;
	    }
	    int projectile = 0;
	    int bulletCount = 1;
	    ...
	    boolean hasShadowPartner = c.getPlayer().getBuffedValue(MapleBuffStat.SHADOWPARTNER) != null;
	    ...
	    boolean soulArrow = c.getPlayer().getBuffedValue(MapleBuffStat.SOULARROW) != null;
	    boolean shadowClaw = c.getPlayer().getBuffedValue(MapleBuffStat.SHADOW_CLAW) != null;
		...
		if (mii.isThrowingStar(projectile)) { //see if player has cash stars
		    MapleInventory cash = c.getPlayer().getInventory(MapleInventoryType.CASH);
		    for (int i = 0; i < 255; i++) { // impose order...
			IItem item = cash.getItem((byte) i);
			if (item != null) {
			    if (item.getItemId() / 1000 == 5021) { //cash stars have prefix 5021xxx
		...
		switch (attack.skill) {
			...
			packet = MaplePacketCreator.rangedAttack(c.getPlayer().getId(), attack.skill, attack.direction, attack.numAttackedAndDamage, visProjectile, attack.allDamage, attack.speed);
			break;
		    default:
			packet = MaplePacketCreator.rangedAttack(c.getPlayer().getId(), attack.skill, attack.stance, attack.numAttackedAndDamage, visProjectile, attack.allDamage, attack.speed);
			break;
		}
		...
		if (c.getPlayer().getJob().isA(MapleJob.ASSASSIN)) {
		    ISkill criticalthrow = SkillFactory.getSkill(4100001);
		    int critlevel = c.getPlayer().getSkillLevel(criticalthrow);
		    if (critlevel > 0) {
			critdamagerate = (criticalthrow.getEffect(c.getPlayer().getSkillLevel(criticalthrow)).getDamage() / 100.0);
		    }
		} else if (c.getPlayer().getJob().isA(MapleJob.BOWMAN)) {
		    ISkill criticalshot = SkillFactory.getSkill(3000001);
		    int critlevel = c.getPlayer().getSkillLevel(criticalshot);
		    if (critlevel > 0) {
		...
		if (hasShadowPartner) {
		    ...
		    MapleStatEffect shadowPartnerEffect = shadowPartner.getEffect(c.getPlayer().getSkillLevel(shadowPartner));
		    if (attack.skill != 0) {
		...
		applyAttack(attack, c.getPlayer(), maxdamage, bulletCount);
	    }
	}
}